function Cal_UTCI(content){
  // Get inputs for UTCI
  var Ta = Number(content.ta); // Air tmperature in Celcius
  var Tmrt = Number(content.ts); // Mean radiant tmperature in Celcius
  var va = Number(content.ws); // Wind velocity in m/s
  var RH = Number(content.hm); // Relative humidity in %

  var g = [-2836.5744, -6028.076559, 19.54263612, -0.02737830188,0.000016261698, (7.0229056*(10**(-10))), (-1.8680009*(10**(-13)))];
  var tk = Ta + 273.15;
  ...
  UTCIapprox = ((Math.round(UTCIapprox*10))/10);
  
  return UTCIapprox;
}

export default Cal_UTCI;